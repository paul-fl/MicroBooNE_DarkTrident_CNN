...
class MPID_Dataset(Dataset):
    def __init__(self, input_file, image_tree, device, plane=0, augment=False, verbose=False):
        ...
        elif "ncpi0_only" in input_file:         self.class_label = 1
        elif "cosmics_corsika" in input_file:    self.class_label = 2
        elif "dm_corsika" in input_file:         self.class_label = 3
        elif "dm_signal_only" in input_file:     self.class_label = 4
        else:
            raise ValueError(f"Unknown file label in: {input_file}")
